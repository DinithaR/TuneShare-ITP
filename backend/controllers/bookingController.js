...
export const generateOwnerBookingsReport = async (req, res) => {
    try {
        ...
        // Helper to generate friendly short refs like O1234/B1234/A1234
    ...
    const friendlyOwner = formatRef(ownerId, 'O');
    const reportRef = formatRef(`${ownerId}-${Date.now()}`, 'R');

        // Start PDF
        const doc = new PDFDocument({ margin: 50, size: 'A4' });
        res.setHeader('Content-Type', 'application/pdf');
        res.setHeader('Content-Disposition', `attachment; filename="owner_bookings_${friendlyOwner}.pdf"`);
        doc.pipe(res);

    // Use the same brand accent as other reports for consistent header styling
    const primaryColor = '#ec4899';
        const muted = '#555555';

    // Header (standardized)
    drawReportHeader(doc, { subtitle: 'Instrument Booking Report', accent: primaryColor });
    doc.fillColor(muted).fontSize(10).text(`Generated: ${new Date().toLocaleString()}`);
    doc.fillColor(muted).fontSize(10).text(`Owner ID: ${friendlyOwner}`);
    doc.fillColor(muted).fontSize(10).text(`Report ID: ${reportRef}`);
    if (role === 'admin') doc.text(`Generated by: ADMIN (${formatRef(_id, 'A')})`);
    doc.moveDown(0.8);
        ...
        const row = (values) => {
            ...
            values.forEach((v, i) => {
                ...
                doc.text(txt, 50 + i*100, doc.y, { continued: i !== values.length -1, width: 100 });
            });
            doc.moveDown(0.4);
        ...
        const renderSection = (label, list) => {
            ...
            tableHeader(['ID', 'Instrument', 'Dates', 'Amount', 'Status']);
            list.forEach(b => {
                ...
                const dates = `${fmtDate(b.pickupDate)}\n${fmtDate(b.returnDate)}`;
                const status = `${(b.status||'').toLowerCase()}${b.paymentStatus==='paid' ? '\npaid' : ''}${b.cancelledAt ? '\n(cancelled)' : ''}`;
                const amount = Number.isFinite(Number(b.price)) ? Number(b.price).toFixed(2) : '—';
                row([
                    formatRef(b._id, 'B'),
                    inst,
                    dates,
        ...
        sectionTitle('Summary');
                const totalRevenue = bookings
                    ...
                    .reduce((sum,b)=> sum + (Number.isFinite(Number(b.price)) ? Number(b.price) : 0), 0)
                    .toFixed(2);
